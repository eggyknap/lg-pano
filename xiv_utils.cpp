  ...
  for(int i=0;i<n;i++)
      ...
      for(int c=0;c<3;c++)
	{
	  ...
	  switch(c)
	    ...
	    case 0:
	      histr[val]++;
	      if(histr[val]>histMax) histMax=histr[val];
	      break;
	    case 1:
	      histg[val]++;
	      if(histg[val]>histMax) histMax=histg[val];
	      break;
	    case 2:
	      histb[val]++;
	      if(histb[val]>histMax) histMax=histb[val];
	      break;
	    }
	}
    }
...
int orientation(const char* file)
{
  ...
  if (d!=NULL)
      ...
      if(e!=NULL)
	{
	  ...
	  if(0==strcmp(tmp,"top - left")) val=0;
	  else if(0==strcmp(tmp,"left - bottom")) val=1;
	  else if(0==strcmp(tmp,"bottom - right")) val=2;
	  else if(0==strcmp(tmp,"right - top")) val=3;
	}
      exif_data_unref (d);
    }
#endif
